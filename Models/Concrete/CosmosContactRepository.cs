...
namespace ContactsCoreMVC.Models.Concrete
{
    public class CosmosContactRepository : IContactRepository
    {
        ...
        public async Task<ContactTable> FindContactAsync(string id)
        {
            ...
            FeedIterator<ContactTable> queryResultSetIterator = _container.GetItemQueryIterator<ContactTable>(queryDefinition);

            List<ContactTable> contactTableList = new List<ContactTable>();

            while (queryResultSetIterator.HasMoreResults)
            {
                FeedResponse<ContactTable> currentResultSet = await queryResultSetIterator.ReadNextAsync();
        ...
        {
            ItemResponse<ContactTable> contactResponse = await _container.ReadItemAsync<ContactTable>(contactTable.Id, new PartitionKey(contactTable.ContactName));
